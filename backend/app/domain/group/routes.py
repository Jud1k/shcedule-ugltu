...
router = APIRouter(
    prefix="/group", tags=["Groups👩‍💻👨‍💻"], dependencies=[Depends(get_current_admin_user)]
)
...
async def get_all_groups(service: GroupServiceDep):
    return await service.get_all()


@router.get("/{group_id}", response_model=GroupRead)
async def get_group_by_id(group_id: int, service: GroupServiceDep):
...
async def create_group(group_in: GroupCreate, service: GroupServiceDep):
    return await service.create(group_in=group_in)


@router.put("/{group_id}", response_model=GroupRead)
async def update_group(group_id: int, group_in: GroupUpdate, service: GroupServiceDep):
