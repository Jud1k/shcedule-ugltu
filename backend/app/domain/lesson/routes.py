...
async def get_all_lessons_by_query(
    ...
    if filters_count == 0:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="At least one filter is required"
        )
    if filters_count > 1:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Only one filter parameter is allowed"
        )
    if group:
...
async def update_lesson(lesson_id: int, lesson_in: LessonUpdate, service: LessonServiceDep):
