...
async def get_all_rooms(service: RoomServiceDep):
    return await service.get_all()


@router.get("/{room_id}", response_model=RoomRead)
async def get_room_by_id(room_id: int, service: RoomServiceDep):
...
async def create_room(room_in: RoomCreate, service: RoomServiceDep):
    return await service.create(room_in=room_in)


@router.put("/{room_id}", response_model=RoomReadMinimal)
async def update_room(room_id: int, room_in: RoomUpdate, service: RoomServiceDep):
