...
import { useLessonsByTeacher } from '../api/get-lessons';
...
export const TeacherLessonsList = ({ teacherId }: TeacherLessonsListProps) => {
  const { selectedDayWeek, setHasLessonsOnDays } = useCalendar();

  const lessonsQuery = useLessonsByTeacher({
    teacherId,
